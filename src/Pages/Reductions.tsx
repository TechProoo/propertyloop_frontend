...
const reductions = [
  {
    ...
    drop: "11%",
    beds: 4, baths: 3, sqft: "6,800", rating: 4.9,
    type: "sale" as const,
  ...
  },
  {
    ...
    drop: "9%",
    beds: 4, baths: 3, sqft: "3,800", rating: 4.9,
    type: "sale" as const,
  ...
  },
  {
    ...
    drop: "9%",
    beds: 6, baths: 5, sqft: "12,000", rating: 4.9,
    type: "sale" as const,
  ...
  },
  {
    ...
    original: "₦75,000,000",
    drop: "17%",
    beds: 3, baths: 2, sqft: "2,100", rating: 4.7,
    type: "sale" as const,
  ...
  },
  {
    ...
    original: "₦55,000,000",
    drop: "13%",
    beds: 3, baths: 2, sqft: "1,800", rating: 4.6,
    type: "sale" as const,
  ...
  },
  {
    ...
    drop: "20%",
    beds: 2, baths: 2, sqft: "1,600", rating: 4.8,
    type: "rent" as const,
    agent: "Prime Realty Lagos",
    lastReduced: "Mar 2026",
    history: [
  ...
  },
  {
    ...
    drop: "20%",
    beds: 0, baths: 4, sqft: "8,500", rating: 4.7,
    type: "sale" as const,
    agent: "Cityscape Properties",
    lastReduced: "Mar 2026",
    history: [
  ...
  },
  {
    ...
    drop: "9%",
    beds: 7, baths: 6, sqft: "15,000", rating: 4.9,
    type: "sale" as const,
...
const Reductions = () => {
  ...
  const filtered = reductions.filter((r) => {
    const matchesType = activeFilter === "All" || r.type === activeFilter.toLowerCase();
    const q = searchQuery.toLowerCase();
    const matchesSearch = !q || r.title.toLowerCase().includes(q) || r.address.toLowerCase().includes(q);
      ...
      <main className="w-full px-6 md:px-12 lg:px-20 pt-5 pb-0">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-text-secondary text-sm mb-8">
            ...
            <span>/</span>
            <span className="text-primary-dark font-medium">Price Reductions</span>
          </div>

          {/* ─── Hero ─── */}
          <div className="relative overflow-hidden rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.06)] mb-10">
            <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url(https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=1400&h=600&fit=crop)" }} />
            ...
            <div className="relative z-10 p-8 sm:p-10 lg:p-14">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-white/70 text-sm font-medium tracking-wide uppercase">Price Drops</p>
              </div>
              ...
              <p className="text-white/60 text-sm leading-relaxed mt-3 max-w-xl">
                Properties and rentals with verified price reductions. Full price history on every listing — no hidden inflation, just transparent pricing.
              </p>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              {filters.map((f) => (
                <button key={f} onClick={() => setActiveFilter(f)} className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${activeFilter === f ? "bg-primary text-white border-primary" : "bg-white/80 text-primary-dark border-border-light hover:border-primary hover:text-primary"}`}>
                  {f}
                </button>
              ))}
              <span className="text-text-secondary text-sm ml-2">
                <span className="font-bold text-primary-dark">{filtered.length}</span> properties
              </span>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-7 mb-20">
            {filtered.map((prop) => (
              <div key={prop.id} className="group bg-white/80 backdrop-blur-sm border border-border-light rounded-[20px] shadow-[0_4px_20px_rgba(0,0,0,0.06)] hover:shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                <div className="flex flex-col sm:flex-row">
                  ...
                  <div className="sm:w-52 h-48 sm:h-auto shrink-0 overflow-hidden relative">
                    ...
                    <span className="absolute bottom-3 left-3 flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/90 backdrop-blur-sm text-primary-dark text-xs font-medium">
                      <Star className="w-3 h-3 text-[#F5A623] fill-[#F5A623]" /> {prop.rating}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 p-5">
                    ...
                    <div className="flex items-baseline gap-3 mb-1">
                      <p className="font-heading font-bold text-primary-dark text-xl">{prop.current}</p>
                      <p className="text-text-subtle text-sm line-through">{prop.original}</p>
                    </div>

                    <h3 className="font-heading font-bold text-primary-dark text-[15px] leading-snug">{prop.title}</h3>
                    ...
                    {prop.beds > 0 && (
                      <div className="flex items-center gap-4 text-text-secondary text-xs mt-3">
                        ...
                        <span className="flex items-center gap-1"><Maximize className="w-3.5 h-3.5" /> {prop.sqft}m²</span>
                      </div>
                    )}
                    ...
                    {/* Price timeline */}
                    <p className="text-text-subtle text-[10px] uppercase tracking-wide mb-2">Price History</p>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <span className="text-text-subtle text-[11px] flex items-center gap-1">
                        <Calendar className="w-3 h-3" /> Reduced {prop.lastReduced}
                      </span>
                      <span className="text-text-secondary text-[11px]">{prop.agent}</span>
                    </div>
                  </div>
                </div>
              </div>
