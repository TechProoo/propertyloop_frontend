          ...
          <div className="flex flex-col lg:flex-row gap-8 mb-20">
                  ...
                  <div className="flex gap-2 p-3">
                    {product.images.map((img, i) => (
                      <button
                        ...
                        className={`h-16 w-24 rounded-xl overflow-hidden shrink-0 border-2 transition-all ${activeImage === i ? "border-primary shadow-lg" : "border-transparent opacity-60 hover:opacity-100"}`}
                      >
                        <img
                          src={img}
                          alt={`View ${i + 1}`}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  {product.specs.map((s) => (
                    <div
                      key={s.label}
                      className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/60 border border-border-light"
                    >
                      <span className="text-text-secondary text-sm">
                        {s.label}
                      </span>
                      <span className="ml-auto font-heading font-medium text-primary-dark text-sm">
                        {s.value}
                      </span>
                    </div>
                  ))}
            ...
            <div className="lg:w-100 shrink-0">
              ...
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.1, duration: 0.4, ease }}
                className="bg-white/70 backdrop-blur-md border border-white/40 rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-6 mb-6 sticky top-8"
              >
                <span className="px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-medium">
                  {product.category}
                </span>
                <h1 className="font-heading font-bold text-primary-dark text-xl mt-3">
                  {product.name}
                </h1>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-0 bg-white/80 border border-border-light rounded-2xl overflow-hidden">
                      <button
                        ...
                        className="w-11 h-11 flex items-center justify-center text-primary-dark hover:bg-bg-accent transition-colors"
                      >
                      ...
                      <input
                        ...
                        className="w-16 h-11 text-center font-heading font-bold text-primary-dark text-sm bg-transparent outline-none border-x border-border-light"
                      />
                      <button
                ...
                <button
                  ...
                  className="mt-5 w-full h-12 rounded-full bg-primary text-white font-bold text-sm hover:bg-primary-dark transition-colors inline-flex items-center justify-center gap-2 shadow-[0_4px_16px_rgba(31,111,67,0.3)]"
                >
                ...
                <div className="flex flex-col gap-2 text-xs text-text-secondary">
                  <div className="flex items-center gap-2">
                    <Truck className="w-4 h-4 text-primary" /> Delivery: 2–5
                    business days within Lagos
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Package className="w-4 h-4 text-primary" /> Bulk orders
                    available · Contact supplier
                  </div>
                </div>
              </motion.div>
              ...
              {similar.length > 0 && (
                ...
                >
                  ...
                  <div className="p-4 flex flex-col gap-3">
                    {similar.map((s) => (
                      <Link
                        key={s.id}
                        to={`/product/${s.id}`}
                        className="group flex gap-4 bg-white/60 border border-border-light rounded-2xl p-3 hover:shadow-[0_4px_20px_rgba(0,0,0,0.08)] hover:-translate-y-0.5 transition-all duration-300"
                      >
                  ...
                  <div className="px-4 pb-4">
                    <Link
                      to="/marketplace"
                      className="w-full h-10 rounded-full bg-white/80 border border-border-light text-primary-dark text-sm font-medium hover:bg-primary hover:text-white hover:border-primary transition-all duration-300 inline-flex items-center justify-center gap-2"
                    >
