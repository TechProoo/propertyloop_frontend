...
import rentalApplicationsService from "../api/services/rentalApplications";
import type { Listing, RentalApplication } from "../api/types";
...
const RentalEscrow = () => {
  ...
  const [leaseDuration] = useState("12");
  ...
  const rental = listing
    ? {
        ...
        period: listing.period ?? "year",
        beds: listing.beds,
        baths: listing.baths,
        sqft: listing.sqft,
        rating: listing.rating,
        agent: listing.agent?.name ?? "Property Loop",
        agency: "Property Loop",
      }
    : fallbackRental;

  const stepIndex = steps.indexOf(currentStep);
  const deposit = rental.price;
  const agencyFee = Math.round(rental.price * 0.1);
  const legalFee = 150000;
  const serviceFee = Math.round(rental.price * 0.05);
  ...
  const goNext = async () => {
    if (currentStep === "payment") {
      setProcessing(true);
      try {
        if (listingId && !application) {
          // Create application + mark deposit paid
          const app = await rentalApplicationsService.create({
            listingId,
          ...
          await rentalApplicationsService.markDepositPaid(app.id);
          setApplication(app);
        }
    ...
    if (currentStep === "esign") {
      ...
      try {
        if (application) {
          await rentalApplicationsService.sign(application.id);
        }
            ...
            <div className="flex-1">
              <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="bg-white/70 backdrop-blur-md border border-white/40 rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-6 sm:p-8">
                      ...
                      <div className="flex flex-col gap-3 text-sm">
                        {[
                          ...
                          {
                            label: "Listing Agent",
                            value: rental.agent + " · " + rental.agency,
                          },
                        ].map((r) => (
                          <div
                            key={r.label}
                          ...
                          >
                            ...
                            <span className="font-heading font-medium text-primary-dark text-right">
                              {r.value}
                            </span>
                          </div>
                        ))}
                    ...
                    <div className="bg-white/70 backdrop-blur-md border border-white/40 rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-6 sm:p-8">
                      ...
                      <div className="flex items-center gap-3 mb-5 px-4 py-3 rounded-2xl bg-primary/5 border border-primary/20">
                        ...
                        <p className="text-primary-dark text-sm">
                          ...
                          until lease signing is complete. If the deal falls
                          through, you get a full refund.
                        </p>
                      </div>
                      <div className="flex flex-col gap-3 text-sm">
                        <div className="flex justify-between py-2">
                          <span className="text-text-secondary">
                            Rental deposit (1 year)
                          </span>
                  ...
                  {currentStep === "esign" && (
                    <div className="bg-white/70 backdrop-blur-md border border-white/40 rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-6 sm:p-8">
                      <h2 className="font-heading font-bold text-primary-dark text-lg mb-2">
                        Sign Tenancy Agreement
                      </h2>
                      <p className="text-text-secondary text-sm mb-5">
                        Payment received. Now sign your tenancy agreement via
                        DocuSeal to complete the process.
                      </p>
                      <div className="bg-white/80 border border-border-light rounded-2xl p-6 mb-5">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            ...
                            <p className="text-text-secondary text-xs">
                              PropertyLoop Standard Lease · 12 months
                            </p>
                          </div>
                        </div>
                        <div className="bg-[#f5f7f5] rounded-xl p-4 border border-border-light text-xs text-text-secondary leading-relaxed h-40 overflow-y-auto">
                          ...
                          <p>
                            This Tenancy Agreement ("Agreement") is entered into
                            on this 1st day of May, 2026 between the Landlord,
                            represented by {rental.agent} of {rental.agency},
                            and the Tenant.
                          </p>
                          <p className="mt-2">
                          ...
                          <p className="mt-1">
                            ...
                            12 months commencing May 1, 2026
                          </p>
                          <p className="mt-2">
                            The Tenant agrees to maintain the property in good
                            condition, pay rent promptly, and comply with estate
                            rules. The Landlord agrees to ensure the property is
                            habitable and all facilities are functional...
                          </p>
                          <p className="mt-2 text-primary-dark font-semibold">
                            Signed via DocuSeal — PropertyLoop Digital Signing
                          </p>
                        </div>
                      </div>
                  ...
                  {currentStep === "confirmed" && (
                    <div className="bg-white/70 backdrop-blur-md border border-white/40 rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-8 sm:p-10 text-center">
                      ...
                      <h2 className="font-heading font-bold text-primary-dark text-xl">
                        Lease Signed & Deposit Secured!
                      </h2>
                      <p className="text-text-secondary text-sm mt-2 max-w-md mx-auto">
                        Your tenancy agreement has been signed and ₦
                        {total.toLocaleString()} is held in escrow. The agent
                        will release the keys on your move-in date.
                      </p>
              ...
              <div className="bg-white/70 backdrop-blur-md border border-white/40 rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.06)] overflow-hidden sticky top-8">
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-3 text-text-secondary text-xs mt-2">
                    ...
                    <span className="flex items-center gap-1">
                      <Bed className="w-3 h-3" /> {rental.beds}
                    </span>
                    <span className="flex items-center gap-1">
                      <Bath className="w-3 h-3" /> {rental.baths}
                    </span>
                  </div>
                  <div className="h-px bg-border-light my-3" />
                  <p className="font-heading font-bold text-primary-dark text-lg">
                    {rental.priceLabel ?? `₦${rental.price.toLocaleString()}`}{" "}
                  ...
                  <div className="flex items-center gap-2 text-xs text-primary">
                    <Shield className="w-4 h-4" /> Escrow-protected deposit
                  </div>
                  <div className="flex items-center gap-2 text-xs text-primary mt-1.5">
                    <FileSignature className="w-4 h-4" /> E-signing via DocuSeal
                  </div>
                </div>
              </div>
            </div>
